...
from ui.dialogs.advanced_dialogs import VideoInfoDialog, TimeInputDialog, AudioProcessingDialog
...
class VideoEditorApp(QMainWindow):
    def __init__(self):
        ...
        self.audio_processor = AudioProcessor(self.engine.ffmpeg.ffmpeg_path or "ffmpeg")
    ...
    def _setup_menu(self):
        ...
        self._action(file_menu, "Open Video", self._open_video, QKeySequence.Open)
        file_menu.addSeparator()
        self._action(file_menu, "Save Project", self._save_project, QKeySequence.Save)
        self._action(file_menu, "Load Project", self._load_project)
        file_menu.addSeparator()
        ...
        self._action(tools_menu, "Batch Processing\u2026", self._open_batch)
    ...
    def _apply_preset(self, options: ProcessingOptions):
        ...
        cp.audio_combo.setCurrentIndex(audio_map.get(options.audio_channels, 0))
        cp.mp3_quality_combo.setCurrentIndex({0: 0, 5: 1, 9: 2}.get(options.mp3_quality, 1))
        cp.codec_copy.setChecked(options.codec_copy)
        cp.use_gpu.setChecked(options.use_gpu)
        cp.video_format_combo.setCurrentText(options.output_format)
        cp.parallel_processing.setChecked(options.parallel_processing)
        cp.max_workers.setValue(options.max_workers)
